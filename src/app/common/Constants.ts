...
export class Constants {
    ...
    public static GetImagePath(mediaStockImageID: number, mediaStockImageTypeID: number, mediaStockImageExt: string = 'jpg'): string {

        if (mediaStockImageID === 0)
            return '';
        if (mediaStockImageTypeID > 0) {
            return this.NANA_IMAGES_DOMAIN + '/upload/mediastock/img/' + mediaStockImageTypeID + '/' + mediaStockImageID / 1000000 +
                '/' + (mediaStockImageID % 1000000) / 1000 + '/' + mediaStockImageID + '.' + mediaStockImageExt;
        }
        return this.NANA_IMAGES_DOMAIN + '/upload/mediastock/NOREPLICATION/img/' + mediaStockImageID / 1000000 + '/'
            + (mediaStockImageID % 1000000) / 1000 + '/' + mediaStockImageID + '_THUMBNAIL.' + mediaStockImageExt;
